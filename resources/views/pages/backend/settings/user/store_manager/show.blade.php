...
<?php 
$auditors              	= $auditor->ondate([(!is_null(Auth::user()->last_logged_at) ? Auth::user()->last_logged_at->format('Y-m-d H:i:s') : null), ' + 7 hours '])/*->type(['voucher_added', 'point_added', 'quota_added', 'price_changed'])*/->userid($authentication->id)->with(['user'])->get();
?>
