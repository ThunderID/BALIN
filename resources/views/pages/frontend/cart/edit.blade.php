...
@section('content')
	<div class="container mt-75">
		...
		<div class="row">
			...
			@endif
			{!! Form::open(['url' => route('frontend.cart.update')]) !!}
				<div class="@if(Auth::User()) col-md-10 col-lg-10 col-sm-9 @else col-xs-12 col-md-12 col-sm-12 @endif">
					...
					<div class="row" style="background-color:black;">
						<div class="hidden-lg hidden-md hidden-sm col-xs-12" >
							...
							<div class="row">
								<div class="col-xs-12">
									<a href="{{ route('frontend.cart.index') }}" class="btn-hollow hollow-white btn-block">
										Batal
									</a>
								</div>
							</div>
							...
							<div class="row">
								<div class="col-xs-12">
									{!! Form::submit('Simpan', ['class' => 'btn-hollow hollow-white btn-block']) !!}
								</div>
							</div>
							<div class="clearfix">&nbsp;</div>
							<div class="clearfix">&nbsp;</div>
						...
						<div class="col-lg-12 col-md-12 col-sm-12 hidden-xs">
							...
							<div class="row">
								<div class="col-lg-12 col-md-12 col-sm-12">
									<a href="{{ route('frontend.cart.index') }}" class="btn-hollow hollow-black pull-right m-r-sm">
										Batal
									</a>
									{!! Form::submit('Simpan', ['class' => 'btn-hollow hollow-black pull-right m-r-sm']) !!}
								</div>
								<div class="clearfix">&nbsp;</div>
								<div class="clearfix">&nbsp;</div>
							</div>
...
@section('script')
	$(window).load(function() {
		var w = $(window).width();
		if (w<767) {
			$('.product-qty-mobile').removeAttr('disabled');
			$('.product-qty-desktop').attr('disabled', 'disabled');
		} else {
			$('.product-qty-desktop').removeAttr('disabled');
			$('.product-qty-mobile').attr('disabled', 'disabled');
		}
	});
	$('.product-qty-desktop').change( function(e) {
		e.preventDefault;
		var total								= 0;
	...
	$('.product-qty-mobile').change( function(e) {
		e.preventDefault;
		var total								= 0;
		var product_price						= parseInt($(this).parent().parent().parent().parent().find('.label-price').attr('data-product-price')); 
		var product_promo_price					= parseInt($(this).parent().parent().parent().parent().find('.label-promo-price').attr('data-product-promo-price')); 
		var product_discount_price				= parseInt($(this).parent().parent().parent().parent().find('.label-discount-price').attr('data-product-discount-price'));
		var product_qty							= parseInt($(this).val());
		
		total 									=  product_qty*product_price;
		$(this).parent().parent().parent().parent().find('.label-total').text('Rp '+number_format(total));
	});
@stop
